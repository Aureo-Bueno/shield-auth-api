...
export class AuthService {
  ...
  constructor(
    ...
    const maxBackoffMsRaw =
      this.configService.get<string>('AUTH_LOGIN_BACKOFF_MAX_MS') ?? '2000';
    const maxBackoffMs = Number(maxBackoffMsRaw);
    if (
      ...
      throw new Error(
        'AUTH_LOGIN_BACKOFF_MAX_MS must be a non-negative integer',
      );
    }
  ...
  async login(
    ...
    await this.applyProgressiveDelay(normalizedEmail, normalizedIp);
  ...
  private async applyProgressiveDelay(
    email: string,
    ipAddress: string,
  ): Promise<void> {
    const emailFailCount = this.emailLoginAttempts.get(email)?.failCount ?? 0;
    const ipFailCount = this.ipLoginAttempts.get(ipAddress)?.failCount ?? 0;
    const failCount = Math.max(emailFailCount, ipFailCount);
    const delayMs = this.calculateBackoffMs(failCount);
    if (delayMs > 0) {
      await sleep(delayMs);
    }
  }

  private calculateBackoffMs(failCount: number): number {
    if (failCount <= 1 || this.maxBackoffMs === 0) {
      return 0;
    }

    const exponentialDelay = 100 * 2 ** (failCount - 1);
    return Math.min(exponentialDelay, this.maxBackoffMs);
  }
}
